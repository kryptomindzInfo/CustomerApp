...
class SendMoneyToWallet extends StatefulWidget {
  ...
  const SendMoneyToWallet({Key key, this.contact}) : super(key: key);
...
class _SendMoneyToWalletState extends State<SendMoneyToWallet> {
  ...
  void initState() {
    ...
    if(widget.contact!=null){
      _getUserDetails = GetUserDetailsApi().getUserDetails(localData.token, widget.contact['mobile']);
      setState(() {
              ...
              child: SizedBox(
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    ...
                    TextFormField(
                      initialValue: widget.contact!=null?widget.contact['mobile']:null,
                      maxLength: 10,
                        ...
                        builder: (context, snapshot) {
                          if(snapshot.hasData){
                            if(mobile==localData.data.mobile){
                              ...
                              if(snapshot.data.status==1 && snapshot.data.user.status==1){
                                List<Map> user = [];
                          ...
                          if(_formKey.currentState.validate()){
                            if(agreeTandC){
                              if(accountExists){
                                  ...
                                  requestModel.sendingAmount = double.parse(amount);
